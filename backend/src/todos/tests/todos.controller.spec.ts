...
describe('TodosController', () => {
    ...
    it('GET /todos, should call findAllTodos with correct parameters and return expected result', async () => {
        const limit = 15;
        const offset = 0;
        const expectedResult = [{ id: '1', name: 'Sample Todo', subject: 'Work', priority: 5, date: new Date(), isCompleted: false, geometryType: 'Point', lat: 0, lng: 0, coordinates: null }];
    ...
    it('GET /todos/filter, should call findAllTodos with correct parameters and filterGeometry and return expected result', async () => {
        const limit = 15;
        const offset = 0;
        const filterGeometry = '{"type":"Polygon","coordinates":[[[3819839.3045508224,3906525.9832158852],[4085504.140501627,3929457.091701438],[3968960.3264575778,3823973.992667895],[3819839.3045508224,3906525.9832158852]]]}'
        const expectedResult = [{ id: '1', name: 'Sample Todo', subject: 'Work', priority: 5, date: new Date(), isCompleted: false, geometryType: 'Point', lat: 0, lng: 0, coordinates: null }];

        mockTodosService.findAllTodos.mockResolvedValue(expectedResult);

        const response = await todosController.findAllFiltered(filterGeometry, limit, offset);

        expect(mockTodosService.findAllTodos).toHaveBeenCalledWith(limit, offset, filterGeometry);
        expect(response).toEqual(expectedResult);
    })
